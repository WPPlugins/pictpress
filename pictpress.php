...
function pp_add_pictures ($post_ID) {
	...
	$dirname = get_settings('fileupload_realpath') . "/$path";
	...
	if (!$dir) {
		...
		for ($i = 0; $i < $dir->count; $i++) {
			...
			} else if ($i && $pp_thumbs_per_row && 
				   ($i % $pp_thumbs_per_row) == 0) {
				$thumbs .= "$pp_clear";
			}
			...
			if ($no_thumbs) {
				...
				if (get_settings('pp_single_post')) {
					$image->id = $first + $i;
					$newpages[] = 
						pp_make_picture_page($pages, 
								     $image);
					$href = pp_get_link($post_ID, 
							    $image->id);
				} else {
					$image->id = pp_make_picture_post(
							$postdata, 
						     	$image); 
...
function pp_expand_subtitles ($content) {
	...
	$content = preg_replace_callback(
			...
			create_function('$matches',
				'return pp_get_subtitle($matches[1]);'),
			$content);
	$content = preg_replace_callback(
		        "/<!--pp-title *([0-9]*)-->/",
		        create_function('$matches',
				'return pp_get_title($matches[1]);'),
			$content);
	$content = preg_replace_callback(
		        "/<!--pp-thumb +(.+?)-->/",
		        create_function('$matches',
				'return pp_get_thumb($matches[1]);'),
			$content);
	// New style formats
	//
	$content = preg_replace_callback(
